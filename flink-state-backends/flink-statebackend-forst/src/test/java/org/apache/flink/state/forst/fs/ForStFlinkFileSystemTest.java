...
public class ForStFlinkFileSystemTest {
    ...
    public void testByteBufferInputStreamAndOutputStream() throws Exception {
        ...
        ForStFlinkFileSystem forStFlinkFileSystem = new ForStFlinkFileSystem(fileSystem, new FileBasedCache(fileSystem, testFilePathForCache, 3000, 0L, 0L), null);
        Path testFilePath = new Path("/tmp", "test-1");
        ByteBufferWritableFSDataOutputStream outputStream = forStFlinkFileSystem.create(testFilePath);
        ...
        ByteBufferReadableFSDataInputStream inputStream = forStFlinkFileSystem.open(
                testFilePath);
        ...
        Assert.assertFalse(forStFlinkFileSystem.exists(new Path("/tmp/cache/test-1")));
        inputStream.close();
        forStFlinkFileSystem.delete(testFilePath, true);
        Assert.assertFalse(forStFlinkFileSystem.exists(testFilePath));
    }

    @Test
    public void testConcurrentPositionRead() throws Exception {
        LocalFileSystem fileSystem = new LocalFileSystem();
        ForStFlinkFileSystem forStFlinkFileSystem = new ForStFlinkFileSystem(fileSystem, null, null);
    ...
    public void testLRUFileBasedCache() throws IOException {
        ...
        ForStFlinkFileSystem forStFlinkFileSystem = new ForStFlinkFileSystem(fileSystem, new FileBasedCache(fileSystem, testFilePathForCache, 3000, 0L, 10000L), null);
